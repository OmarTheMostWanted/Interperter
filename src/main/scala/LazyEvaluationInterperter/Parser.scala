...
object Parser {
  ...
  def isReserved(s: String): String =
    if(ExprExt.reservedWords.contains(s))
  ...
  def parse(sexpr: SExpr): ExprExt = sexpr match {
    ...
    //let
    case SList(SSym("let") :: SList(binds) :: tail :: Nil) if binds != Nil =>
      LetExt(parseBinders(binds), parse(tail))

    //letrec
    case SList(SSym("letrec") :: SList(binds) :: tail :: Nil ) if binds != Nil =>
    ...
    case SList(SSym(s) :: t1 :: Nil)
      if List("-", "not", "head", "tail", "is-nil", "is-list", "force") contains s =>
      UnOpExt(s, parse(t1))

    //binary operation
    case SList(SSym(s) :: t1 :: t2 :: Nil)
      if List("+", "*", "-", "and", "or", "num=", "num<", "num>", "cons") contains s =>
