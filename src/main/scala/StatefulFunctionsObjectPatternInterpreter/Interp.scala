  ...
  def interp(e: ExprC, nv: PointerEnvironment, st1: Store): (Value, Store) = {
    ...
    e match {
      ...
      case EqStrC(l, r) => {
        ...
        val (right, st3) = interp(r, nv, st2)
        (left, right) match {
          case (StringV(l), StringV(r)) => (BoolV(l == r), st3)
          case _ => throw NotAStringException()
        }
      }
      ...
      case ConcStrC(l, r) => {
        ...
        val (right, st3) = interp(r, nv, st2)
        (left, right) match {
          case (StringV(l), StringV(r)) => (StringV(l + r), st3)
          case _ => throw NotAStringException()
        }
      }
      case IfC(c, t, e) => {
          ...
          case PointerClosV(FdC(params, body), nv_clos) => {
            ...
            if (params.size != args.size) {
              throw ArgumentsDoNotMatchParametersException("for parameters" + params.toString() + " and arguments " + args.toString()
                + "\nthis happened in function " + body + "\n with params " + params + " and args " + args
              )
            }
